...
const ReceiptPickerDemo: React.FC = () => {
  const { colors } = useTheme();
  const [state, setState] = useState<'empty' | 'filled' | 'loading'>('empty');

  return (
    <DemoCard
      title="ReceiptPicker"
      description="거래 추가 폼 내 영수증 첨부 컴포넌트 (issue #11). 빈 상태 → ActionSheet → 첨부 완료 흐름"
    >
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        {(['empty', 'filled', 'loading'] as const).map((s) => (
          <button
            key={s}
            onClick={() => setState(s)}
            style={{
              padding: '6px 12px',
              borderRadius: 8,
              border: `1.5px solid ${state === s ? colors.primary : colors.border}`,
              backgroundColor: state === s ? colors.primaryLight + '33' : 'transparent',
              color: state === s ? colors.primary : colors.textSecondary,
              fontSize: 12,
      ...
      <div style={{ maxWidth: 320 }}>
        ...
        {state === 'filled' && (
          ...
          >
            ...
            >
              <span style={{ fontSize: 12, color: colors.textSecondary }}>receipt_20260307.jpg</span>
              <button
                style={{
                  background: 'none',
                  border: 'none',
        ...
        {state === 'loading' && (
          ...
          >
            ...
            >
              ...
              />
              <span style={{ color: '#fff', fontSize: 12, fontWeight: 600 }}>업로드 중...</span>
            </div>
          </div>
        )}
      </div>
...
export const ReceiptPicker: StoryObj = {
  name: 'ReceiptPicker',
  render: () => <ReceiptPickerDemo />,
};
